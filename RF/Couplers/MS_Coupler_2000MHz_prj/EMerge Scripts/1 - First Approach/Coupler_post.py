...
ax1.plot(f_MHz, dB(S21), label='S21 (Direct path)',     lw=1.8, color='r')  # red
ax1.plot(f_MHz, dB(S31), label='S31 (Coupled path)',   lw=1.8, color='g')    # green
ax1.plot(f_MHz, dB(S41), label='S41 (Isolated path)',   lw=1.8, color='k')    # black
ax1.axvline(f0, color='gray', ls='--', lw=1, label=f'f₀ = {f0} MHz')

# Marker: Direct path IL @ f0
idx_f0    = np.argmin(np.abs(f_MHz - f0))
mkr_at_f0  = dB(S21)[idx_f0]
ax1.annotate(f'Direct path IL = {mkr_at_f0:.1f} dB',
             xy=(f0, mkr_at_f0),
             xytext=(f0, mkr_at_f0-4),
             arrowprops=dict(arrowstyle='->', color='black'),
             fontsize=10)

# Marker: Coupled path IL @ f0
idx_f0    = np.argmin(np.abs(f_MHz - f0))
mkr_at_f0  = dB(S31)[idx_f0]
ax1.annotate(f'Coupling= {-mkr_at_f0:.1f} dB',
             xy=(f0, mkr_at_f0),
             xytext=(f0, mkr_at_f0 + 4),
             arrowprops=dict(arrowstyle='->', color='black'),
             fontsize=10)

# Marker: Isolated path IL @ f0
idx_f0    = np.argmin(np.abs(f_MHz - f0))
mkr_at_f0  = dB(S41)[idx_f0]
ax1.annotate(f'Isolation = {-mkr_at_f0:.1f} dB',
             xy=(f0, mkr_at_f0),
             xytext=(f0 - 100, mkr_at_f0 + 4),
             arrowprops=dict(arrowstyle='->', color='black'),
             fontsize=10)

ax1.set_xlabel('Frequency [MHz]')
ax1.set_ylabel('Magnitude [dB]')
ax1.set_title(f'15 dB MS Coupler — S-parameters')
ax1.set_ylim(-40, 2)
...
# ---------------------------------------------------------------------------
# Figure 2: Phase difference between direct output and coupled port
# ---------------------------------------------------------------------------
...
ax2.set_ylabel('Phase(S21) − Phase(S31) [°]')
ax2.set_title(f'15 dB MS Coupler — Phase difference between direct and coupled paths')
