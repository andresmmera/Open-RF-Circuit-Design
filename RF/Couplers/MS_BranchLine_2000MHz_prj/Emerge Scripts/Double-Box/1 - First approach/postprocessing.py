...
pdf_filename  = project_name + "_report.pdf"
...
rows = [
    ...
    ("W121",             f"{float(d['W121']):.3f} mm"),
    ("Lx  (lambda/4)",   f"{float(d['Lx']):.1f} mm"),
    ("Ly  (lambda/4)",   f"{float(d['Ly']):.1f} mm"),
...
# ---------------------------------------------------------------------------
# Figure 2: Phase difference S21 - S41
# ---------------------------------------------------------------------------
...
ax2.annotate(f'{pd_at_f0:.1f} deg',
             xy=(f0, pd_at_f0), xytext=(f0 + 100, pd_at_f0 + 8),
             arrowprops=dict(arrowstyle='->', color='black'), fontsize=10)

ax2.set_xlabel('Frequency [MHz]')
ax2.set_ylabel('Phase(S21) - Phase(S41) [deg]')
